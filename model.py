...
class FaceModel(nn.Module):
    def __init__(self, opt, isTrain=True, input_nc=3):
        ...
        self.netEncoder = networks.init_net(
                networks.Encoder(in_channels=input_nc), gpu_ids=self.gpu_ids)
        self.netClassifier = networks.init_net(
                networks.FeatEmbedder(), gpu_ids=self.gpu_ids)
        self.netDepthDecoder = networks.init_net(
                networks.Decoder(), gpu_ids=self.gpu_ids)
        self.netDepthDiscriminator = networks.init_net(
                networks.Discriminator(nc=4), gpu_ids=self.gpu_ids)

        self.model_names = ["Encoder", "DepthDecoder", "DepthDiscriminator",
                            "Classifier"]
        ...
        if self.isTrain:
            ...
            self.criterionCls = [torch.nn.CrossEntropyLoss(),
                                 losses.FocalLoss()]
            # net G/
            self.optimizer_depth = torch.optim.Adam(itertools.chain(
                self.netEncoder.parameters(),
                self.netDepthDecoder.parameters()),
                lr=opt.lr, betas=(opt.beta1, 0.999))

            # net D/
            self.optimizer_discriminate = torch.optim.Adam(
                    self.netDepthDiscriminator.parameters(),
                    lr=opt.lr, betas=(opt.beta1, 0.999))

            # net cls
            self.optimizer_cls = torch.optim.Adam(itertools.chain(
                self.netEncoder.parameters(), self.netClassifier.parameters()),
    ...
    def backward_C(self):
        output = self.output
        self.loss_C = (2*self.criterionCls[0](output, self.label)
                       + self.criterionCls[1](output, self.label)) * self.w_cls
        # (self.criterionCls[0](output,self.label)
        #  + self.criterionCls[1](self.cls_feat, self.label))
    ...
    def optimize_parameters(self):
        ...
        if self.model in [f"model{i}" for i in range(1, 4)]:
            self.forward()
            # update C
    ...
    def set_requires_grad(self, nets, requires_grad=False):
        """Set requies_grad=False for all nets to avoid needless computations
        Parameters:
    ...
    def save_networks(self, epoch):
        ...
        Parameters:
            epoch (int) -- current epoch
                used in the file name [epoch]_net_[name].pth
        """
        for name in self.model_names:
            if isinstance(name, str):
    ...
    def get_current_visuals(self):
        ...
        train.py will display these images with visdom, and save the images to
        a HTML
        """
    ...
    def get_current_losses(self):
        ...
        train.py will print out these errors on console, and save them to a
        file
        """
